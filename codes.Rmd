...
### SPSS'te numeric tanımlanmış ve veri değerleri olan sütunlar, labelled
...
genelde verilerde ID sütun unutuluyor. Bazı analizlerde ise ID sütunu mutlaka gerekiyor. Hem datayı yeniden düzenlediğinizde olguları karıştırmamak için hem de bazı paketlerde hatalarla karşılaşmamak için her veri setine ID eklenmesini öneririm.
...
```{r}
data <- data %>%
    select(subject, tani, starts_with("klinik"))
```
